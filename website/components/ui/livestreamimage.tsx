...
export default function LiveStreamImage({
  ...
  const [imgSrc, setImgSrc] = useState("/api/camera/stream");
...
useEffect(() => {
  ...
  (async () => {
    try {
      const r = await fetch("/api/camera/status", { cache: "no-store" });
...
function LockedPoster({ kind }: { kind: "unauth" | "forbidden" }) {
  const msg = kind === "unauth" ? "Sign in to view the live stream" : "Access required";
