...
SOUND_DIR = "/opt/sound-api/sounds"
...
def populate_available_sounds() -> None:
    # Scan /opt/sound-api/sounds directory and populate AVAILABLE_SOUNDS list
    global AVAILABLE_SOUNDS
    
    if not os.path.exists(SOUND_DIR):
        print(f"Warning: Sound directory does not exist: {SOUND_DIR}")
        return
    ...
    try:
        ...
        print(f"Error reading sound directory: {e}")
        return
    
    AVAILABLE_SOUNDS.sort()
    print(f"Loaded {len(AVAILABLE_SOUNDS)} sounds from {SOUND_DIR}")
